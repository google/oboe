...
public:
    ...
    bool transformSample(float sample, float referencePhase) {
        ...
        mSinAccumulator += static_cast<double>(sample) * static_cast<double>(sinf(referencePhase));
        mCosAccumulator += static_cast<double>(sample) * static_cast<double>(cosf(referencePhase));
        mFramesAccumulated++;
