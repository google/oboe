...
namespace iolib {
    ...
    SimpleMultiPlayer::SimpleMultiPlayer()
            : mChannelCount(0), mSampleRate(0), mNumSampleBuffers(0), mOutputReset(false) {}
    ...
    void SimpleMultiPlayer::MyErrorCallback::onErrorAfterClose(AudioStream *oboeStream, Result error) {
        ...
        mParent->resetAll();
        if (mParent->openStream(PerformanceMode::LowLatency) && mParent->startStream()) {
            mParent->mOutputReset = true;
        }
    }
    
    void SimpleMultiPlayer::MyPresentationCallback::onPresentationEnded(oboe::AudioStream *oboeStream) {
        AudioStreamPresentationCallback::onPresentationEnded(oboeStream);
    }

    bool SimpleMultiPlayer::openStream(oboe::PerformanceMode performanceMode) {
    ...
    bool SimpleMultiPlayer::startStream() {
        ...
        while (tryCount < 3) {
            ...
            if (tryCount > 0) {
                // Sleep between tries to give the system time to settle.
                usleep(20 * 1000);
                // Try to open the stream again after the first try.
                wasOpenSuccessful = openStream(PerformanceMode::LowLatency);
            }
